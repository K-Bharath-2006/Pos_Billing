...
Ensure you have your environment set up for React Native CLI development (Node.js, JDK, Android SDK / Android Studio).
...
### 2. Configure Vector Icons (Android)
Font assets copy settings are pre-configured in `android/app/build.gradle`. Ensure you have an active Android emulator or device connected via `adb devices`.

### 3. Start Metro Bundler
Start the Metro server in a separate terminal:
```bash
npx react-native start
```

### 4. Run the Application
Compile and launch on your connected Android emulator/device:
```bash
...
## Release APK Build Steps

Follow these steps to compile a release APK for your store device.

### 1. Generate a Signing Key
If you don't have a release signing key, generate one via keytool inside your JDK bin directory:
```bash
keytool -genkeypair -v -storetype PKCS12 -keystore my-release-key.keystore -alias my-key-alias -keyalg RSA -keysize 2048 -validity 10000
```
Place the generated `.keystore` file in the `android/app/` directory and configure credentials in `android/app/build.gradle` (or use the debug configuration for quick local distribution).

### 2. Compile Release Assembly
...
### 3. Retrieve Compiled APK
Once completed, the release-ready standalone APK can be retrieved from:
